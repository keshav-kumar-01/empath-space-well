...
const Navigation: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t border-slate-200/50 dark:border-slate-700/50 z-50 md:hidden safe-area-inset-bottom shadow-lg">
      <div className="flex justify-around items-center py-1 px-1 max-w-screen-sm mx-auto">
        {navigationItems.map((item) => {
          const IconComponent = item.icon;
          const isActive = location.pathname === item.path;
          
          return (
            <Link key={item.path} to={item.path} className="flex-1 max-w-[16.66%]">
              <Button
                variant="ghost"
                size="sm"
                className={`flex flex-col items-center justify-center gap-0.5 h-14 w-full py-1 px-1 min-w-0 rounded-lg transition-all duration-200 ${
                  isActive 
              ...
              >
                <IconComponent className={`h-4 w-4 shrink-0 ${isActive ? 'animate-pulse' : ''}`} />
                ...
                {isActive && (
                  <div className="w-1 h-1 bg-chetna-primary rounded-full mt-0.5"></div>
                )}
              </Button>
            </Link>
          );
